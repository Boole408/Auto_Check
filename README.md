...
也可以在浏览器手动登录对应站点，例如 `muyuan.do` 或 `anyrouter.top`，然后把 localStorage 里的 `user` JSON 粘贴进来；只要里面包含 `username` 和 `token`，AutoCheck 就会直接使用这个登录态调用 `/api/user/checkin`。
